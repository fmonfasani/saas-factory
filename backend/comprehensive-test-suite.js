...
async function runTests() {
  log('\n=== COMPREHENSIVE SAAS FACTORY API TEST SUITE ===\n', 'blue');
  ...
  // Test 2: API Root
  results.total++;
  const apiRootResult = await testEndpoint('GET', '/');
  if (apiRootResult.success) {
    results.passed++;
  } else {
    results.failed++;
  }
  
  // Test 3: POST /auth/signup
  ...
  if (signinResult.success) {
    ...
    log(`  Access Token obtained: ${accessToken ? `${accessToken.substring(0, 20)}...` : 'None'}`, 'green');
  } else {
    results.failed++;
  }
  
  // Test 5: GET /auth/me (protected)
  results.total++;
  ...
  // Test 7: POST /auth/forgot-password
  results.total++;
  ...
  // Test 8: POST /auth/resend-verification
  results.total++;
  ...
  log('\n--- 3. User Profile Endpoints (Protected) ---', 'blue');
  
  // Test 9: GET /users/profile
  results.total++;
  ...
  log('\n--- 4. Organization Endpoints (Protected) ---', 'blue');
  
  // Test 11: POST /organizations/
  results.total++;
  ...
  if (orgCreateResult.success) {
    results.passed++;
    testOrgId = orgCreateResult.data.id || orgCreateResult.data.data?.id;
    log(`  Organization created with ID: ${testOrgId}`, 'green');
  } else {
    results.failed++;
  }
  
  // Test 12: GET /organizations/
  results.total++;
  ...
  log('\n--- 5. Project Endpoints (Protected) ---', 'blue');
  
  // Test 15: POST /organizations/:orgId/projects/
  if (testOrgId) {
    results.total++;
    ...
    if (projectCreateResult.success) {
      results.passed++;
      testProjectId = projectCreateResult.data.id || projectCreateResult.data.data?.id;
  ...
  // Test 16: GET /organizations/:orgId/projects/
  if (testOrgId) {
    results.total++;
  ...
  // Test 17: GET /organizations/:orgId/projects/:id
  ...
  // Test 18: PUT /organizations/:orgId/projects/:id
  if (testOrgId && testProjectId) {
    results.total++;
  ...
  log('\n--- 6. Generate API Endpoints ---', 'blue');
  
  // Test 19: POST /generate/analyze
  results.total++;
  ...
  // Test 21: GET /generate/status/:jobId (non-existent job)
  results.total++;
  ...
  // Test 22: GET /generate/ab-results
  results.total++;
  ...
  log('\n--- 7. Error Path Tests ---', 'blue');
  
  // Test 23: Signup with duplicate email
  results.total++;
  ...
  // Test 24: Signin with wrong password
  results.total++;
  ...
  // Test 25: Access protected endpoint without token
  results.total++;
  ...
  // Test 28: POST /generate/analyze without prompt
  results.total++;
  ...
  // Test 29: DELETE /organizations/:id (cleanup)
  if (testOrgId) {
    results.total++;
    const orgDeleteResult = await testEndpoint('DELETE', `/organizations/${testOrgId}`, null, {
      'Authorization': `Bearer ${accessToken}`
    });
    if (orgDeleteResult.success) {
      results.passed++;
      log(`  Organization deleted: ${testOrgId}`, 'green');
    } else {
      results.failed++;
    }
  }
...
// Run the test suite
log('\n🚀 Starting Comprehensive API Test Suite...', 'blue');
